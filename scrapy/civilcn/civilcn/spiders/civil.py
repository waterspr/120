...
class CivilSpider(scrapy.Spider):
    ...
    def parse(self, response):
        #获取内层的文章的链接
        urllists = response.xpath('//div[@class="m_g_b_d"]//li/span/a/@href').extract()
        # print(urllists)
        for url in urllists :
            print(url)
        ...
        #构造下一页url
        # next_text=response.xpath('//div[@class="m_g_b_c"]/a[last()-1]/text()').extract_first()
        # if next_text == '下一页':
        #     next_page = response.xpath('//div[@class="m_g_b_c"]/a[last()-1]/@href').extract_first()  
        #     print('-'*35)
        #     print(next_page)
    ...
    def parse2(self, response):
        title = response.xpath('//div[@class="m_g_title"]/text()').extract_first()  
        print(title)
        ...
        for p in ps :
            ...
            if  img :
                ...
                try :
                    document.add_picture('f:/1/'+img_name,width=Inches(6)) #写入图片到word
                except:
                    pass
            ...
            if strong_name :  
                ...
                print(content1)
                try:
                    content1 = content1.replace('\n','').replace('\r','')  #去掉回车符\r和换行符\n两种标记
                except:
                    pass
                document.add_paragraph(content1)
            # #提取文本内容    
            
            contents = p.xpath('./text()').extract() #extract_first()返回是第一个字符串，取不全
